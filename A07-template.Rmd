...
Trips %>%
  ggplot(aes(x = client, y = as.numeric((edate - sdate) / 360 ))) + geom_boxplot() +
  ylim(0,45) +
  ylab("Hours")
...
*Observations*: The data collected reflectes a portion of the 365 days.The peak bike uses were at start of the data collection.
...
My observations here..... 
*Observations*: Week 5 had the highest bike uses and Week 6 had the second highest bike uses. Week 7 had the lowest bike uses. 
...
My observations here..... 
*Observation*: The highest bike uses were are hour 17. There were multiple hours with zero bike uses. 
...
My observations here..... 
*Observation*: Within the minute period, the bike uses never fell below 300. The peak were around 50 second and the low were around 10 second.
...
Trips %>%
  group_by(hours = lubridate::hour(sdate), days = lubridate::wday(sdate), client) %>%
  summarise(count = n()) %>%
  ggplot(aes(x = hours, y = count, color = days)) +
...
My comments here.....
Registered bike user have a higher count in bike uses compared to casual bike users. There is a rise and dip in bike uses for both of the causal and registered around 10th hour mark to the 20th hour mark. Registered bike uses were high around the 7.5 hour mark and 17.5 hour mark.
...
inner_join(Trips, PairDistances, by = c("sstation", "estation")) %>%
